...
NUM_REGIONS = 12  # Number of regions in the Glasser atlas
...
def get_filtered_task_betas(task_betas, df_conditions, first_delay_only=False, second_delay_only=False):
    delay_task_betas, delay_task_df = filter_task_betas(task_betas, df_conditions, phase="delay", first_delay_only=first_delay_only, second_delay_only=second_delay_only)
    encoding_task_betas, encoding_task_df = filter_task_betas(task_betas, df_conditions, phase="encoding", first_delay_only=first_delay_only, second_delay_only=second_delay_only)
...
def decode_regions(sessions, glasser_atlas, delay_task_betas, delay_task_df, encoding_task_betas, encoding_task_df, tasks, decoding_feature, phase, classifier, feature_normalization, random_state = 42):
    ...
    for region in range(0, NUM_REGIONS + 1):
...
# Save results
def save_results(regionwise_acc, t_stats_map, p_values_map, tasks, decoding_feature, phase, first_delay_only=False, second_dealy_only = False, rep=1):
    if not first_delay_only:
        results_dir = Path(f"{PARENT_DIR}/network_level_results/{'_'.join(tasks)}_{decoding_feature}_{phase}")
    elif second_dealy_only:
        results_dir = Path(f"{PARENT_DIR}/network_level_results/{'_'.join(tasks)}_{decoding_feature}_{phase}_second_delay_only")
    else:
        results_dir = Path(f"{PARENT_DIR}/network_level_results/{'_'.join(tasks)}_{decoding_feature}_{phase}_first_delay_only")
...
def main(subj="sub-03", tasks=None, decoding_feature="category", 
         ...
         first_delay_only=False, rep=1, second_delay_only=False, random_state = 42):
    
    if tasks is None:
        tasks = ['interdmsctgABAB', 'interdmsctgABBA']  # Default tasks
    ...
    delay_task_betas, delay_task_df, encoding_task_betas, encoding_task_df = get_filtered_task_betas(task_betas, df_conditions, first_delay_only=first_delay_only, second_delay_only=second_delay_only)
    ...
    # Save results
    save_results(regionwise_acc, t_stats_map, p_values_map, tasks, decoding_feature, phase, first_delay_only=first_delay_only, second_dealy_only=second_delay_only, rep=rep)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--decoding_feature", type=str, default="category", help="Decoding feature (e.g., 'category' or 'location').")
